...
gulp.task("renameVersionJson", ["renameIndexJs"] , function () {
	...
	if (config.version) {
		...
		.on('end', function () {
			...
			indexName = path.join(releaseDir, versionCon['index.js'] || "index.js");
			indexName = indexName.split('\\').pop();
			var bundleJSPath = path.join(releaseDir, versionCon['js/bundle.js'] || "js/bundle.js");
			bundleName = bundleJSPath.split('\\').pop();
			versionName = `${versionCon['version.json']}`;
			bundleJSStr = fs.readFileSync(bundleJSPath, "utf8");
			bundleJSStr = bundleJSStr.replace('ResourceVersion.enable(\"version.json\"', `ResourceVersion.enable("${versionName}"`);
			bundleJSStr = bundleJSStr.replace('version.config_version_md5_name.json', `${versionName}`);
...
gulp.task("packfile", platformTask, function() {
	...
	if (platform === 'wxgame') {
		...
		// 命令打包加入bundle文件
		const bundStr = `require("js/${bundleName}");`;
		if (gameJsCon.indexOf(bundStr) === -1) {
			gameJsCon += bundStr;
		}
		// 修改game.js
		const domStr = 'window.Parser=require("js/dom_parser");';
		if (gameJsCon.indexOf(domStr) === -1) {
			gameJsCon += domStr;
		}
		fs.writeFileSync(gameJsPath, gameJsCon, "utf8");
	}
	else if (platform === 'web') {
		let gameJsPath = path.join(releaseDir, `${indexName}`);
		let gameJsCon = fs.readFileSync(gameJsPath, "utf8");
		// 命令打包加入bundle文件
		const bundStr = `loadLib("js/${bundleName}")`;
		if (gameJsCon.indexOf(bundStr) === -1) {
			gameJsCon += `${bundStr};`
		}
		fs.writeFileSync(gameJsPath, gameJsCon, "utf8");
