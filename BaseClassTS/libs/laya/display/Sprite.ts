...
export class Sprite extends Node {
	...
	/**
	 * 自定义截图绘制纹理，drawToTexture存在ctx内存泄漏该方法提供ctx及tex缓存
	 * by yanmingjie
	 * @param sprite
	 * @param canvasWidth
	 * @param canvasHeight
	...
	static drawToTextureCustom(
		...
		ctx?: Context,
		tex?: Texture
	): { texture: Texture, ctx: Context } {
		const _renderType = sprite._renderType;
        offsetX -= sprite.x;
        offsetY -= sprite.y;
        offsetX |= 0;
        offsetY |= 0;
        canvasWidth |= 0;
        canvasHeight |= 0;
		const curCtx = ctx ? ctx: new Context();
		curCtx.clear();
		curCtx.size(canvasWidth, canvasHeight);
		curCtx.asBitmap=true;
		if(curCtx._targets){
			curCtx._targets.start();
			curCtx._targets.clear(0, 0, 0, 0);	// 否则没有地方调用clear
			RenderSprite.renders[_renderType]._fun(sprite, curCtx, offsetX, offsetY);
			curCtx.flush();
			curCtx._targets.end();
			curCtx._targets.restore();
		}
		const rtex = tex ? tex: new Texture();
		rtex.setTo(((<Texture2D>(curCtx._targets as any))), Texture.INV_UV);
		return { texture: rtex, ctx: curCtx };
    }
