...
export class Graphics {
    ...
    clearDrawSubmit(draw: DrawImageCmd): void {
        const cmds = this._cmds;
        // 有且只有一个渲染命令
        if (this._one === draw && cmds && cmds.length <= 0) {
            this.clear(true);

            // 有多个命令
        } else if (this._one === draw && cmds && cmds.length > 0) {
            const tCmd = this._one;
            tCmd.recover();

            this._one = null;
            this._repaint();
        } else if (cmds) {
            let i: number, len = cmds.length;
            let tCmd: DrawImageCmd;
            for (i = 0; i < len; i++) {
                tCmd = cmds[i];
                if (tCmd === draw) {
                    cmds.splice(i, 1);
            ...
            if (this._cmds.length <= 0 && !this._one) {
                this.clear(true);
            }
   ...
   * @param height	（可选）高度。
   */
    drawImage2(texture: Texture, x: number = 0, y: number = 0, width: number = 0, height: number = 0): DrawImageCmd | null {
        if (!texture) return null;
        if (!width) width = texture.sourceWidth;
        if (!height) height = texture.sourceHeight;
        if (texture.getIsReady()) {
