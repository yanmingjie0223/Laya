...
export class ResourceVersion {
    ...
    static addVersionPrefix(originURL: string): string {
		let newOriginURL: string = originURL;
		if (Config.useWebp && originURL.indexOf('.webp') !== -1) {
			const pngUrl = originURL.replace('.webp', '.png');
			if (ResourceVersion.manifest && ResourceVersion.manifest[pngUrl]) {
				newOriginURL = pngUrl;
			}
			else {
				const jpgUrl = originURL.replace('.webp', '.jpg');
				if (ResourceVersion.manifest && ResourceVersion.manifest[jpgUrl]) {
					newOriginURL = jpgUrl;
				}
			}
		}

        if (ResourceVersion.manifest && ResourceVersion.manifest[newOriginURL]) {
            if (ResourceVersion.type == ResourceVersion.FILENAME_VERSION) return ResourceVersion.manifest[newOriginURL];
            return ResourceVersion.manifest[newOriginURL] + "/" + newOriginURL;
        }
