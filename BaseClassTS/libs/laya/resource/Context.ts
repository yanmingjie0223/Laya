...
export class Context {
	...
	static set2DRenderConfig(): void {
		...
		// https://ask.layabox.com/article/10
		// by yanmingjie 添加两行修复ios15黑屏问题
	...
	/**add custom shader batch by yanmingjie */
	private _customShaderId: number;
	...
	_fillTexture(texture: Texture, texw: number, texh: number, texuvRect: any[], x: number, y: number, width: number, height: number, type: string, offsetx: number, offsety: number): void {
		...
		if (!this.clipedOff(this._transedPoints)) {
			...
			// add custom shader by yanmingjie
			const custom2d = texture['customValue2D'];
			var sv: Value2D = custom2d ? custom2d : Value2D.create(ShaderDefines2D.TEXTURE2D, 0);
			// src code
	...
	*/
	...
	_useNewTex2DSubmit(tex: Texture, minVertNum: number): void {
		...
		// add custom shader by yanmingjie
		const custom2d = tex['customValue2D'];
		var submit = SubmitTexture.create(this, this._mesh, custom2d ? custom2d : Value2D.create(ShaderDefines2D.TEXTURE2D, 0));
	...
	_inner_drawTexture(tex: Texture, imgid: number, x: number, y: number, width: number, height: number, m: Matrix, uv: ArrayLike<number>|null, alpha: number, lastRender: boolean): boolean {
		...
		/**
		 * add custom shader batch by yanmingjie
		 * 如果shader customId不同 视为另一个shader
		 */
		const custom2d = tex['customValue2D'];
		if (sameKey) {
			if (custom2d) {
				const customId = custom2d.customId;
				if (this._customShaderId !== customId) {
					sameKey = false;
					this._customShaderId = customId;
				}
			}
			else if (this._customShaderId) {
				this._customShaderId = 0;
				sameKey = false;
			}
		}
		else {
			if (custom2d) {
				this._customShaderId = custom2d.customId;
			}
			else {
				this._customShaderId = 0;
			}
...

		{
			...
			if (!sameKey) {
				...
				// add custom shader by yanmingjie
				this._submits[this._submits._length++] = this._curSubmit = submit = SubmitTexture.create(this, mesh, custom2d ? custom2d : Value2D.create(ShaderDefines2D.TEXTURE2D, 0));
				submit.shaderValue.textureHost = tex;
				// imgid相同不能合并
				if (custom2d && custom2d.getSplitBatch && custom2d.getSplitBatch()) {
					submit._key.other = -this._customShaderId;
				}
				else {
					submit._key.other = imgid;
				}
	...
	drawTriangles(tex: Texture,
		...
		if (!sameKey) {
			...
			// add custom shader by yanmingjie
			var submit: SubmitTexture = this._curSubmit = SubmitTexture.create(this, triMesh, custom2d ? custom2d : Value2D.create(ShaderDefines2D.TEXTURE2D, 0));
			submit.shaderValue.textureHost = tex;
			submit._renderType = SubmitBase.TYPE_TEXTURE;
