...
export class Texture2D extends BaseTexture {
	...
	/**是否立即解码 by: yanmingjie */
	static IS_DECODE: boolean = true;
	 ...
	 * 设置之后纹理宽高可能会发生变化。
	 */
	loadImageSource(source: any, premultiplyAlpha: boolean = false, forceDecode: boolean = false): void {
		// by yanmingjie 这里存储延后程序中手动调用解析资源
		if (!forceDecode) {
			if (!Texture2D.IS_DECODE) {
				SystemUtils.addNoDecodeTexture(this, source, premultiplyAlpha);
				return;
			}
		}
