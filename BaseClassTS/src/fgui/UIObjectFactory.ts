...
export class UIObjectFactory {
	...
	public static setPackageItemExtension(
		url: string,
		type: new () => GComponent
	): void {
	...
	public static resolvePackageItemExtension(pi: PackageItem): void {
		var extensionType =
			UIObjectFactory.extensions["ui://" + pi.owner.id + pi.id];
		if (!extensionType)
			extensionType =
				UIObjectFactory.extensions["ui://" + pi.owner.name + "/" + pi.name];
...
Decls.UIObjectFactory = UIObjectFactory as any;