...
import { updateScaler } from "./UIContentScaler";
...
export class GRoot extends GComponent {
	...
	constructor() {
		...
		this._cKey = GObjectType.GRoot;
		if (!GRoot._inst) GRoot._inst = this;
	...
	public showModalWait(msg?: string): void {
		if (UIConfig.globalModalWaiting != null) {
			if (this._modalWaitPane == null)
				this._modalWaitPane = UIPackage.createObjectFromURL(
					UIConfig.globalModalWaiting
				);
	...
	public showPopup(
		...
		if (target) {
			...
			pos = this.globalToLocal(stage.mouseX, stage.mouseY);
		}
		...
		yy = pos.y + sizeH;
		if (
			((dir === undefined || dir === PopupDirection.Auto) &&
				pos.y + popup.height > this.height) ||
			dir === false ||
		...
		const pxy = this.globalToLocal(xx, yy);
		popup.setXY(pxy.x, pxy.y);
	}
	...
	public playOneShotSound(url: string, volumeScale?: number): void {
		if (ToolSet.startsWith(url, "ui://")) return;
	...
	private __addedToStage(): void {
		stage.on(Event.MOUSE_DOWN, this, this.__stageMouseDown);
		stage.on(Event.MOUSE_UP, this, this.__stageMouseUp);
	...
	public checkPopups(clickTarget: Sprite): void {
		if (this._checkPopups) return;

		this._checkPopups = true;
		this._justClosedPopups.length = 0;
		if (this._popupStack.length > 0) {
			var mc: Node = clickTarget;
	...
	private __stageMouseDown(evt: Event): void {
