...
import { Decls } from "./GObjectConst";
...
export class Transition {
	...
	private _play(
		...
		for (var i: number = 0; i < cnt; i++) {
			...
			if (item.target == null) {
				...
				item.target.parent != this._owner
			)
				item.target = null;

			if (item.target && item.type == ActionType.Transition) {
				var trans: Transition = (<GComponent>item.target).getTransition(
	...
	private stopItem(item: Item, setToComplete: boolean): void {
		...
		if (item.tweener) {
			...
			if (item.type == ActionType.Shake && !setToComplete) {
				...
				item.target.setXY(
					item.target.x - item.value.lastOffsetX,
					item.target.y - item.value.lastOffsetY
				);
	...
	public setHook(label: string, callback: Handler): void {
		var cnt: number = this._items.length;
		var found: boolean = false;
		for (var i: number = 0; i < cnt; i++) {
			var item: Item = this._items[i];
	...
	public setTarget(label: string, newTarget: GObject): void {
		...
		for (var i: number = 0; i < cnt; i++) {
			...
			if (item.label == label) {
				item.targetId =
					newTarget == this._owner || newTarget == null ? "" : newTarget.id;
				if (this._playing) {
	...
	public onOwnerRemovedFromStage(): void {
		if ((this._options & OPTION_AUTO_STOP_DISABLED) == 0)
			this.stop(
				(this._options & OPTION_AUTO_STOP_AT_END) != 0 ? true : false,
				false
			);
	}
	...
	private internalPlay(): void {
		...
		if (!this._reversed) {
			for (var i: number = 0; i < cnt; i++) {
				...
				if (
					item.type == ActionType.Animation &&
					this._startTime != 0 &&
					item.time <= this._startTime
				) {
	...
	private applyValue(item: Item): void {
		...
		switch (item.type) {
			...
			case ActionType.Color:
				item.target.setProp(
					ObjectPropID.Color,
					ToolSet.convertToHtmlColor(value.f1, false)
				);
				break;
				...
				if (this._playing) {
					...
					if (trans) {
						...
						var startTime: number =
							this._startTime > item.time ? this._startTime - item.time : 0;
						var endTime: number =
							this._endTime >= 0 ? this._endTime - item.time : -1;
						if (
							value.stopTime >= 0 &&
							(endTime < 0 || endTime > value.stopTime)
						)
							endTime = value.stopTime;
						trans.timeScale = this._timeScale;
						trans._play(
							Handler.create(this, this.onPlayTransCompleted, [item]),
							value.playTimes,
							0,
							startTime,
							endTime,
			...
			case ActionType.Shake:
				item.target.setXY(
					item.target.x - value.lastOffsetX + value.offsetX,
					item.target.y - value.lastOffsetY + value.offsetY
				);
			...
			case ActionType.ColorFilter: {
				ToolSet.setColorFilter(item.target.displayObject, [
					value.f1,
					value.f2,
					value.f3,
					value.f4,
						...
						for (var j: number = 0; j < pathLen; j++) {
							...
							switch (curveType) {
								...
								case CurveType.CubicBezier:
									pts.push(
										GPathPoint.newCubicBezierPoint(
											buffer.getFloat32(),
											buffer.getFloat32(),
											buffer.getFloat32(),
											buffer.getFloat32(),
