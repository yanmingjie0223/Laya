...
export class GList extends GComponent {
	...
	public set lineCount(value: number) {
		if (this._lineCount != value) {
			...
			if (
				this._layout == ListLayoutType.FlowVertical ||
	...
	public set columnCount(value: number) {
		if (this._columnCount != value) {
			...
			if (
				this._layout == ListLayoutType.FlowHorizontal ||
				this._layout == ListLayoutType.Pagination
			) {
				this.setBoundsChangedFlag();
				if (this._virtual) this.setVirtualListChangedFlag(true);
	...
	public set virtualItemSize(value: Point) {
		if (this._virtual) {
			if (this._itemSize == null) this._itemSize = new Point();
	...
	public removeChildAt(index: number, dispose?: boolean): GObject {
		...
		else child.off(Event.CLICK, this, this.__clickItem);
	...
	public get selectedIndex(): number {
		...
		if (this._virtual) {
			for (i = 0; i < this._realNumItems; i++) {
				...
				if (
					(ii.obj instanceof GButton && ii.obj.selected) ||
	...
	public getSelection(result?: number[]): number[] {
		if (!result) result = new Array<number>();
		var i: number;
		if (this._virtual) {
			for (i = 0; i < this._realNumItems; i++) {
				var ii: ItemInfo = this._virtualItems[i];
	...
	public handleArrowKey(dir: number): void {
		...
		switch (dir) {
			...
			case 3: //right
				if (
					this._layout == ListLayoutType.SingleRow ||
					this._layout == ListLayoutType.FlowHorizontal ||
					this._layout == ListLayoutType.Pagination
				) {
					index++;
	...
	private __clickItem(evt: Event): void {
		...
		this.dispatchItemEvent(
			item,
			Events.createEvent(Events.CLICK_ITEM, this.displayObject, evt)
		);
	}
	...
	private setSelectionOnEvent(item: GObject, evt: Event): void {
		if (
			!(item instanceof GButton) ||
			this._selectionMode == ListSelectionMode.None
		)
			return;
		...
		if (this._selectionMode == ListSelectionMode.Single) {
			...
			if (evt.shiftKey) {
				...
				evt.ctrlKey ||
				this._selectionMode == ListSelectionMode.Multiple_SingleClick
			) {
	...
	public resizeToFit(itemCount?: number, minSize?: number): void {
		...
		if (this._virtual) {
			...
			if (
				...
				this.viewHeight =
					lineCount * this._itemSize.y +
					Math.max(0, lineCount - 1) * this._lineGap;
			else
				this.viewWidth =
					lineCount * this._itemSize.x +
					Math.max(0, lineCount - 1) * this._columnGap;
		} else if (itemCount == 0) {
			if (
				this._layout == ListLayoutType.SingleColumn ||
				this._layout == ListLayoutType.FlowHorizontal
	...
	private shouldSnapToNext(dir: number, delta: number, size: number): boolean {
		return (
			(dir < 0 && delta > UIConfig.defaultScrollSnappingThreshold * size) ||
			(dir > 0 &&
				delta > (1 - UIConfig.defaultScrollSnappingThreshold) * size) ||
			(dir == 0 && delta > size / 2)
		);
	}

	public getSnappingPositionWithDir(
		xValue: number,
		yValue: number,
		xDir: number,
		yDir: number,
		...
		if (this._virtual) {
			...
			return super.getSnappingPositionWithDir(
				xValue,
				yValue,
				xDir,
				yDir,
	...
	public scrollToView(index: number, ani?: boolean, setFirst?: boolean): void {
		if (this._virtual) {
			...
			if (index >= this._virtualItems.length)
				throw new Error(
					"Invalid child index: " + index + ">" + this._virtualItems.length
				);
			...
			if (
				...
				var page: number =
					index / (this._curLineItemCount * this._curLineItemCount2);
				rect = new Rectangle(
					page * this.viewWidth +
						(index % this._curLineItemCount) * (ii.width + this._columnGap),
					((index / this._curLineItemCount) % this._curLineItemCount2) *
						(ii.height + this._lineGap),
					ii.width,
					ii.height
				);
			}
	...
	public set numItems(value: number) {
		...
		if (this._virtual) {
			...
			if (this._loop) this._realNumItems = this._numItems * 6;
			//设置6倍数量，用于循环滚动
			...
			if (this._virtualListChanged != 0) {
				timer.clear(this, this._refreshVirtualList);
			}

			timer.clearCallLater(this, this._refreshVirtualList);
			//立即刷新
			...
			if (value > cnt) {
				for (i = cnt; i < value; i++) {
					...
					else this.addItemFromPool(this.itemProvider.runWith(i));
				}
			} else {
	...
	private setVirtualListChangedFlag(layoutChanged?: boolean): void {
		...
		timer.clearCallLater(this, this._refreshVirtualList);
		timer.callLater(this, this._refreshVirtualList);
	}
	...
	private _refreshVirtualList(): void {
		...
		if (this._realNumItems > 0) {
			...
			if (
				...
				var pageCount: number = Math.ceil(
					len / (this._curLineItemCount * this._curLineItemCount2)
				);
	...
	private getIndexOnPos3(forceUpdate: boolean): number {
		...
		var startIndex: number =
			page * (this._curLineItemCount * this._curLineItemCount2);
		...
		while (curIndex < this._realNumItems && (end || curY < max)) {
			...
			if (ii.obj == null) {
				...
				if (ii.obj) {
					this.setChildIndex(
						ii.obj,
						forward ? curIndex - newFirstIndex : this.numChildren
					);
				} else {
			...
			if (needRender) {
				if (
					...
					(this._layout == ListLayoutType.SingleColumn || this._columnCount > 0)
				)
				...
				let w: number = ii.obj.width;
				let h: number = ii.obj.height;
				if (curIndex % this._curLineItemCount == 0) {
					if (ii.obj.scaleX != 1) {
						w = ii.obj.width * ii.obj.scaleX;
					}

					if (ii.obj.scaleY != 1) {
						h = ii.obj.height * ii.obj.scaleY;
					}
					deltaSize += Math.ceil(h) - ii.height;
					if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex) {
						//当内容向下滚动时，如果新出现的项目大小发生变化，需要做一个位置补偿，才不会导致滚动跳动
						firstItemDeltaSize = Math.ceil(h) - ii.height;
					}
				}
				ii.width = Math.ceil(w);
				ii.height = Math.ceil(h);
			}
	...
	private handleScroll2(forceUpdate: boolean): boolean {
		...
		while (curIndex < this._realNumItems && (end || curX < max)) {
			...
			if (needRender) {
				if (
					this._autoResizeItem &&
					(this._layout == ListLayoutType.SingleRow || this._lineCount > 0)
				)
	...
	private handleAlign(contentWidth: number, contentHeight: number): void {
		...
		if (
			newOffsetX != this._alignOffset.x ||
			newOffsetY != this._alignOffset.y
		) {
		...
		if (this._layout == ListLayoutType.SingleColumn) {
			...
			if (this._autoResizeItem && this._columnCount > 0) {
				for (i = 0; i < cnt; i++) {
					...
					if (j == this._columnCount || i == cnt - 1) {
						ratio =
							(viewWidth - lineSize - (j - 1) * this._columnGap) / lineSize;
						curX = 0;
				...
				for (i = 0; i < cnt; i++) {
					...
					if (
						(this._columnCount != 0 && j >= this._columnCount) ||
						(this._columnCount == 0 &&
							curX + child.width > viewWidth &&
							maxHeight != 0)
					) {
						//new line
						curX = 0;
			...
			if (this._autoResizeItem && this._lineCount > 0) {
				for (i = 0; i < cnt; i++) {
					...
					if (j == this._lineCount || i == cnt - 1) {
						ratio =
							(viewHeight - lineSize - (j - 1) * this._lineGap) / lineSize;
						curY = 0;
						for (j = lineStart; j <= i; j++) {
							child = this.getChildAt(j);
							if (this.foldInvisibleItems && !child.visible) continue;
				...
				for (i = 0; i < cnt; i++) {
					...
					if (
						(this._lineCount != 0 && j >= this._lineCount) ||
						(this._lineCount == 0 &&
							curY + child.height > viewHeight &&
							maxWidth != 0)
					) {
						curY = 0;
						curX += Math.ceil(maxWidth) + this._columnGap;
			...
			if (this._autoResizeItem && this._lineCount > 0)
				eachHeight = Math.floor(
					(viewHeight - (this._lineCount - 1) * this._lineGap) / this._lineCount
				);
			...
			if (this._autoResizeItem && this._columnCount > 0) {
				for (i = 0; i < cnt; i++) {
					...
					if (
						j == 0 &&
						((this._lineCount != 0 && k >= this._lineCount) ||
					...
					if (j == this._columnCount || i == cnt - 1) {
						...
						for (j = lineStart; j <= i; j++) {
							...
							if (j < i) {
								child.setSize(
									child.sourceWidth + Math.round(child.sourceWidth * ratio),
									this._lineCount > 0 ? eachHeight : child.height,
									true
								);
								curX += Math.ceil(child.width) + this._columnGap;
							} else {
