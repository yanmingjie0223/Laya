...
import { Decls, GObjectType } from "./GObjectConst";
...
const GearClasses: Array<typeof GearBase> = [
	GearDisplay,
	GearXY,
	GearSize,
	GearLook,
...
export class GObject {
	...
	protected _cKey: GObjectType;
	protected _displayObject: Sprite;
	...
	constructor() {
		this._cKey = GObjectType.GObject;
	...
	public setXY(xv: number, yv: number): void {
		if (this._x != xv || this._y != yv) {
			...
			if (this._cKey === GObjectType.GGroup) (<any>this).moveChildren(dx, dy);

			this.updateGear(1);

			if (this._parent && !(this._parent._cKey === GObjectType.GList)) {
		...
		if (this._rawWidth != wv || this._rawHeight != hv) {
			...
			if (this._pivotX != 0 || this._pivotY != 0) {
				if (!this._pivotAsAnchor) {
					if (!ignorePivot)
						this.setXY(
							this.x - this._pivotX * dWidth,
							this.y - this._pivotY * dHeight
						);
					this.updatePivotOffset();
				} else this.applyPivot();
			}

			if (this._cKey === GObjectType.GGroup) (<any>this).resizeChildren(dWidth, dHeight);
			...
			if (this._parent) {
				this._relations.onOwnerSizeChanged(
					dWidth,
					dHeight,
					this._pivotAsAnchor || !ignorePivot
				);
				this._parent.setBoundsChangedFlag();
	...
	private updatePivotOffset(): void {
		if (this._displayObject) {
			if (
				...
				var pt: Point = this._displayObject.transform.transformPoint(
					sHelperPoint
				);
	...
	public set touchable(value: boolean) {
		if (this._touchable != value) {
			...
			if (
				this._cKey === GObjectType.GImage ||
				this._cKey === GObjectType.GMovieClip ||
				this._cKey === GObjectType.GTextField ||
	...
	private __rollOver(evt: Event): void {
		timer.once(100, this, this.__doShowTooltips);
	}
	...
	private __rollOut(evt: Event): void {
		timer.clear(this, this.__doShowTooltips);
	...
	public getGear(index: number): GearBase {
		...
		if (!gear) this._gears[index] = gear = new GearClasses[index](this);
		return gear;
	}
	...
	private checkGearDisplay(): void {
		...
		var connected: boolean =
			!this._gears[0] || (<GearDisplay>this._gears[0]).connected;
		if (this._gears[8])
	...
	public get displayObject(): Sprite {
	...
	public get root(): GRoot {
		if (this._cKey === GObjectType.GRoot) return <any>this;

		var p: GObject = this._parent;
		while (p) {
			if (p._cKey === GObjectType.GRoot) return <any>p;
			p = p.parent;
		}
	...
	public onClick(listener: Function, thisObj: any, args?: any[]): void {
		this.on(Event.CLICK, thisObj, listener, args);
	}

	public offClick(listener: Function, thisObj: any): void {
	...
	public localToGlobal(ax?: number, ay?: number, result?: Point): Point {
		ax = ax || 0;
		ay = ay || 0;
	...
	public globalToLocal(ax?: number, ay?: number, result?: Point): Point {
		ax = ax || 0;
		ay = ay || 0;
		result = result || new Point();
		result.x = ax;
	...
	public setup_beforeAdd(buffer: ByteBuffer, beginPos: number): void {
		...
		if (BlendMode[bm]) this.blendMode = BlendMode[bm];
	...
	public setup_afterAdd(buffer: ByteBuffer, beginPos: number): void {
		...
		if (groupId >= 0) this.group = <GGroup>this.parent.getChildAt(groupId);
	...
	private initDrag(): void {
		if (this._draggable) this.on(Event.MOUSE_DOWN, this, this.__begin);
		else this.off(Event.MOUSE_DOWN, this, this.__begin);
	}
	...
	private dragBegin(touchID?: number): void {
		if (GObject.draggingObject) {
			...
			Events.dispatch(Events.DRAG_END, tmp._displayObject, {
				touchId: touchID,
			});
		}

		sGlobalDragStart.x = stage.mouseX;
		sGlobalDragStart.y = stage.mouseY;

		this.localToGlobalRect(0, 0, this.width, this.height, sGlobalRect);
		this._dragTesting = true;
		GObject.draggingObject = this;

		stage.on(Event.MOUSE_MOVE, this, this.__moving);
		stage.on(Event.MOUSE_UP, this, this.__end);
	}
	...
	private reset(): void {
		stage.off(Event.MOUSE_MOVE, this, this.__moving);
		stage.off(Event.MOUSE_UP, this, this.__end);
	}

	private __begin(): void {
		if (!this._dragStartPos) this._dragStartPos = new Point();
		this._dragStartPos.x = stage.mouseX;
		this._dragStartPos.y = stage.mouseY;
		this._dragTesting = true;
	...
	private __moving(evt: Event): void {
		if (
			GObject.draggingObject != this &&
			this._draggable &&
			this._dragTesting
			...
			if (
				this._dragStartPos &&
				Math.abs(this._dragStartPos.x - stage.mouseX) < sensitivity &&
				Math.abs(this._dragStartPos.y - stage.mouseY) < sensitivity
			)
				return;
		...
		if (GObject.draggingObject == this) {
			var xx: number = stage.mouseX - sGlobalDragStart.x + sGlobalRect.x;
			var yy: number = stage.mouseY - sGlobalDragStart.y + sGlobalRect.y;

			if (this._dragBounds) {
				var rect: Rectangle = Decls.GRoot.inst.localToGlobalRect(
					this._dragBounds.x,
					this._dragBounds.y,
	...
	public static cast(sprite: Sprite): GObject {
...
let _gInstanceCounter: number = 0;
let sGlobalDragStart: Point = new Point();
let sGlobalRect: Rectangle = new Rectangle();
let sHelperPoint: Point = new Point();
let sDragHelperRect: Rectangle = new Rectangle();
let sUpdateInDragging: boolean;
let sDraggingQuery: boolean;
