...
export class GBasicTextField extends GTextField {
	...
	constructor() {
		...
		this._cKey = GObjectType.GBasicTextField;
		this._text = "";
		this._color = "#000000";
	...
	public set text(value: string) {
		...
		if (this._bitmapFont == null) {
			if (this._widthAutoSize) this._textField.width = 10000;
	...
	public set font(value: string) {
		...
		else delete this._bitmapFont;

		if (this._bitmapFont) {
			this._textField["setChanged"]();
		} else {
			if (this._font) this._textField.font = this._font;
			else this._textField.font = UIConfig.defaultFont;
		}
	}
	...
	public set fontSize(value: number) {
		const oldSize = this._textField.fontSize;
	...
	public set color(value: string) {
		if (this._color != value) {
			...
			if (this.grayed) this._textField.color = "#AAAAAA";
			else this._textField.color = this._color;
		}
	}
	...
	protected handleXYChanged(): void {
		...
		const that: any = this;
		let xv: number = that._x;
		let yv: number = that._y + that._yOffset;
		if (that._pivotAsAnchor) {
			xv -= that._pivotX * that._width;
			yv -= that._pivotY * that._height;
		}
		if (that._pixelSnapping) {
			xv = Math.round(xv);
			yv = Math.round(yv);
		}

		// by: yanmingjie 文本向上偏移，这里将位置调整回来
		let ty = yv + that._pivotOffsetY;
		if (!that._font) {
			ty += that.fontSize * 0.12;
		}
		else if (that._font.indexOf('ui') === -1) {
			ty += that.fontSize * 0.12;
		}

		that._displayObject.pos(xv + that._pivotOffsetX, ty);
	}
	...
	protected updateAutoSize(): void {
		...
		if (!this._underConstruct) {
			if (!this._heightAutoSize) this._textField.size(this.width, this.height);
			else if (!this._widthAutoSize) this._textField.width = this.width;
		}
	}
	...
	public typeset(): void {
		if (this._bitmapFont) this.renderWithBitmapFont();
		else if (this._widthAutoSize || this._heightAutoSize) this.updateSize();
	}
	...
	private renderWithBitmapFont(): void {
		...
		if (!this._lines) this._lines = new Array<LineInfo>();
		else returnList(this._lines);
		...
		var fontScale: number = this._bitmapFont.resizable
			? fontSize / this._bitmapFont.size
			: 1;
		this._textWidth = 0;
		this._textHeight = 0;
		...
		var w: number,
			h: number = 0;
		if (this._widthAutoSize) {
			if (this._textWidth == 0) w = 0;
			else w = this._textWidth;
		} else w = this.width;

		if (this._heightAutoSize) {
			if (this._textHeight == 0) h = 0;
			else h = this._textHeight;
		} else h = this.height;
		...
		for (var i: number = 0; i < lineCount; i++) {
			...
			if (this.align == "center") lineIndent = (rectWidth - line.width) / 2;
			else if (this.align == "right") lineIndent = rectWidth - line.width;
			else lineIndent = 0;
			...
			for (var j: number = 0; j < textLength; j++) {
				...
				if (glyph) {
					charIndent =
						(line.height + line.textHeight) / 2 -
						Math.ceil(glyph.lineHeight * fontScale);
					if (glyph.texture) {
						gr.drawTexture(
							glyph.texture,
	...
	protected handleSizeChanged(): void {
		...
		if (this._underConstruct) this._textField.size(this._width, this._height);
		else {
			if (this._bitmapFont) {
				if (!this._widthAutoSize) this._textField["setChanged"]();
				else this.doAlign();
			} else {
				if (!this._widthAutoSize) {
					if (!this._heightAutoSize)
						this._textField.size(this._width, this._height);
					else this._textField.width = this._width;
				}
			}
		}
	}
	...
	private doAlign(): void {
		if (this.valign == "top" || this._textHeight == 0) this._yOffset = GUTTER_Y;
		else {
			var dh: number = this.height - this._textHeight;
			if (dh < 0) dh = 0;
			if (this.valign == "middle") this._yOffset = Math.floor(dh / 2);
			else this._yOffset = Math.floor(dh);
		}
		this.handleXYChanged();
	}
