...
export class GGraph extends GObject {
	...
	private updateGraph(): void {
		...
		if (/*Render.isWebGL &&*/ ToolSet.startsWith(fillColor, "rgba")) {
			//webgl下laya未支持rgba格式
			var arr: any[] = fillColor
				.substring(5, fillColor.lastIndexOf(")"))
				.split(",");
			var a: number = parseFloat(arr[3]);
			if (a == 0) fillColor = null;
			else {
				fillColor = Utils.toHexColor(
		...
		if (this._type == 1) {
			if (this._cornerRadius) {
				...
				gr.drawPath(
					...
					0,
					paths,
					fillColor ? { fillStyle: fillColor } : null,
					this._lineSize > 0
						? { strokeStyle: lineColor, lineWidth: this._lineSize }
						: null
				);
			} else
				gr.drawRect(
					...
					h,
					fillColor,
					this._lineSize > 0 ? lineColor : null,
					this._lineSize
				);
	...
	public setNativeObject(obj: Sprite): void {
		this._type = 0;
		this._displayObject.mouseEnabled = this.touchable;
	...
	public getProp(index: number): any {
		...
		else return super.getProp(index);
	}

	public setProp(index: number, value: any): void {
		if (index == ObjectPropID.Color) this.color = value;
		else super.setProp(index, value);
	}
	...
	public setup_beforeAdd(buffer: ByteBuffer, beginPos: number): void {
		...
		if (this._type != 0) {
			...
			if (buffer.readBool()) {
				this._cornerRadius = [];
				for (i = 0; i < 4; i++) this._cornerRadius[i] = buffer.getFloat32();
			}
			...
			if (this._type == 3) {
				...
				for (i = 0; i < cnt; i++) this._polygonPoints[i] = buffer.getFloat32();
			} else if (this._type == 4) {
				...
				if (cnt > 0) {
					this._distances = [];
					for (i = 0; i < cnt; i++) this._distances[i] = buffer.getFloat32();
				}
			}
