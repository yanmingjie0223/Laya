...
export class GLoader3D extends GObject {
	...
	public constructor() {
		...
		this._cKey = GObjectType.GLoader3D;
		this._playing = true;
		this._url = "";
	...
	public set color(value: string) {
		if (this._color != value) {
			...
			if (this._content) ToolSet.setColorFilter(this._content, this._color);
		}
	}
	...
	protected loadContent(): void {
		...
		if (ToolSet.startsWith(this._url, "ui://")) this.loadFromPackage(this._url);
	...
	protected loadFromPackage(itemURL: string) {
		...
		if (this._contentItem) {
			...
			if (
				this._contentItem.type == PackageItemType.Spine ||
				this._contentItem.type == PackageItemType.DragonBones
			)
	...
	public setSkeleton(skeleton: Skeleton, anchor?: Point): void {
		this.url = null;
	...
	private onChange(): void {
		...
		if (this._animationName) {
			if (this._playing) this._content.play(this._animationName, this._loop);
			else
				this._content.play(
					this._animationName,
					false,
		...
		if (this._skinName) this._content.showSkinByName(this._skinName);
		else this._content.showSkinByIndex(0);
	}
	...
	private updateLayout(): void {
		...
		if (this._align == AlignType.Center) nx = Math.floor((this.width - cw) / 2);
		else if (this._align == AlignType.Right) nx = this.width - cw;
		else nx = 0;
		if (this._verticalAlign == VertAlignType.Middle)
			ny = Math.floor((this.height - ch) / 2);
		else if (this._verticalAlign == VertAlignType.Bottom) ny = this.height - ch;
		else ny = 0;
