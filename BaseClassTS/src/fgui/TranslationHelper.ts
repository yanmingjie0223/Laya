...
export class TranslationHelper {
	...
	public static translateComponent(item: PackageItem): void {
		...
		var compStrings: { [index: string]: string } =
			TranslationHelper.strings[item.owner.id + item.id];
		...
		for (i = 0; i < childCount; i++) {
			...
			for (j = 0; j < gearCnt; j++) {
				...
				if (buffer.readByte() == 6) {
					...
					if (
						buffer.readBool() &&
						(value = compStrings[elementId + "-texts_def"]) != null
					)
						buffer.writeS(value);
				}
			...
			if (baseType == ObjectType.Component && buffer.version >= 2) {
				...
				for (var k: number = 0; k < cpCount; k++) {
					...
					if (
						propertyId == 0 &&
						(value = compStrings[elementId + "-cp-" + target]) != null
					)
						buffer.writeS(value);
					else buffer.skip(2);
				}
			...
			switch (type) {
				...
				case ObjectType.Tree: {
					...
					for (j = 0; j < itemCount; j++) {
						...
						buffer.skip(2); //url
						if (type == ObjectType.Tree) buffer.skip(2);
				...
				case ObjectType.Label: {
					if (buffer.seek(curPos, 6) && buffer.readByte() == type) {
						...
						if (
							buffer.readBool() &&
							(value = compStrings[elementId + "-prompt"]) != null
						)
							buffer.writeS(value);
					}
					break;
