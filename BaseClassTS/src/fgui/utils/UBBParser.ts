export class UBBParser {
	...
	constructor() {
		...
		this._handlers["br"] = this.onTag_BR;
	}

	protected onTag_URL(tagName: string, end: boolean, attr: string): string {
		if (!end) {
			if (attr != null) return '<a href="' + attr + '" target="_blank">';
			else {
	...
	protected onTag_FONT(tagName: string, end: boolean, attr: string): string {
		if (!end) return '<span style="font-family:' + attr + '">';
		else return "</span>";
	}
	...
	protected onTag_BR(tagName: string, end: boolean, attr: string): string {
		if (end) {
			return "<br/>";
		}
		return '';
	...
	public parse(text: string, remove?: boolean): string {
		...
		while ((pos2 = this._text.indexOf("[", pos1)) != -1) {
			...
			if (func != null) {
				...
				// 这里需要记录颜色
				else if (func === this.onTag_COLOR && attr) {
					this.lastColor = attr;
				}
