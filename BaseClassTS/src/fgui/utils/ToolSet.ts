...
export class ToolSet {
	...
	public static convertFromHtmlColor(str: string, hasAlpha?: boolean): number {
		...
		if (str.length == 8)
			return (
				(parseInt(str.substr(0, 2), 16) << 24) + parseInt(str.substr(2), 16)
			);
		else if (hasAlpha) return 0xff000000 + parseInt(str, 16);
		else return parseInt(str, 16);
	}

	public static displayObjectToGObject(obj: Node): GObject {
		while (obj && !(obj instanceof Stage)) {
	...
	public static setColorFilter(
		obj: Sprite,
		color?: string | number[] | boolean
	): void {
		var filter: ColorFilter = (<any>obj).$_colorFilter_; //cached instance
		...
		if (tp == "boolean") {
			...
			if (tp == "string") {
				...
				if (arr[0] == 1 && arr[1] == 1 && arr[2] == 1) color = null;
				else {
					color = [
						arr[0],
						0,
		...
		if (toApplyGray) filter.gray();
		else if (toApplyColor.length == 20) filter.setByMatrix(toApplyColor);
		else
			filter.setByMatrix(
				getColorMatrix(
					toApplyColor[0],
