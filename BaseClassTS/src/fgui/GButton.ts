...
export class GButton extends GComponent {
	...
	constructor() {
		...
		this._cKey = GObjectType.GButton;
	...
	public set icon(value: string) {
		this._icon = value;
		value =
			this._selected && this._selectedIcon ? this._selectedIcon : this._icon;
	...
	public set selected(val: boolean) {
		...
		if (this._selected != val) {
			...
			if (
				...
				if (this._selected) this.setState(GButton.SELECTED_DISABLED);
				else this.setState(GButton.DISABLED);
			} else {
				if (this._selected)
					this.setState(this._over ? GButton.SELECTED_OVER : GButton.DOWN);
				else this.setState(this._over ? GButton.OVER : GButton.UP);
			}
	...
	public getTextField(): GTextField {
		if (this._titleObject instanceof GTextField) return this._titleObject;
			...
			this._titleObject['_cKey'] === GObjectType.GLabel  ||
			this._titleObject instanceof GButton
		) {
	...
	public fireClick(downEffect?: boolean): void {
		...
		if (downEffect && this._mode == ButtonMode.Common) {
			...
			timer.once(100, this, this.setState, [GButton.DOWN], false);
			timer.once(200, this, this.setState, [GButton.UP], false);
		}
	...
	protected setState(val: string): void {
		...
		if (this._downEffect == 1) {
			...
			if (
				val == GButton.DOWN ||
				val == GButton.SELECTED_OVER ||
				val == GButton.SELECTED_DISABLED
			) {
	...
	protected handleGrayedChanged(): void {
		if (
			...
			} else if (this._selected) this.setState(GButton.DOWN);
	...
	private __mousedown(evt: Event): void {
		...
		stage.on(Event.MOUSE_UP, this, this.__mouseup);
		...
		if (this._linkedPopup) {
			...
			else this.root.togglePopup(this._linkedPopup, this);
		}
	}

	private __mouseup(): void {
		if (this._down) {
			stage.off(Event.MOUSE_UP, this, this.__mouseup);
			this._down = false;
			...
			if (this._mode == ButtonMode.Common) {
				...
				else if (this._over) this.setState(GButton.OVER);
				else this.setState(GButton.UP);
			}
		}
	}
