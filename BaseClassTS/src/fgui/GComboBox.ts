...
export class GComboBox extends GComponent {
	...
	constructor() {
		...
		this._cKey = GObjectType.GComboBox;
	...
	public getTextField(): GTextField {
		if (this._titleObject instanceof GTextField) return this._titleObject;
			...
			this._titleObject instanceof GLabel ||
	...
	protected showDropdown(): void {
		...
		if (this._popupDirection == PopupDirection.Down) downward = true;
		else if (this._popupDirection == PopupDirection.Up) downward = false;
	...
	private __popupWinClosed(): void {
		if (this._over) this.setState(GButton.OVER);
		else this.setState(GButton.UP);
	}

	private __clickItem(itemObject: GObject, evt: Event): void {
		timer.callLater(this, this.__clickItem2, [
			this._list.getChildIndex(itemObject),
			evt,
		]);
	}

	private __clickItem2(index: number, evt: Event): void {
	...
	private __mousedown(evt: Event): void {
		if (evt.target instanceof Input) return;
