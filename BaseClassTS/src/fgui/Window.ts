...
export class Window extends GComponent {
	...
	public set dragArea(value: GObject) {
		if (this._dragArea != value) {
			...
			if (this._dragArea) {
				if (this._dragArea['_cKey'] === GObjectType.GGraph) {
					(<GGraph>this._dragArea).drawRect(0, null, null);
				}
	...
	public hideImmediately(): void {
		let r: GRoot = this.parent['_cKey'] === GObjectType.GRoot ? this.parent as any : null;
		if (!r) r = Decls.GRoot.inst;
	...
	protected layoutModalWaitPane(): void {
		if (this._contentArea) {
			...
			this._modalWaitPane.setXY(
				pt.x + this._contentArea.x,
				pt.y + this._contentArea.y
			);
