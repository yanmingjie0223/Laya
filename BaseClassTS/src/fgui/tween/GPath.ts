...
export class GPath {
	...
	public create(
		...
		if (Array.isArray(pt1)) points = pt1;
		else {
		...
		for (var i: number = 1; i < cnt; i++) {
			...
			} else splinePoints.push(new Point(current.x, current.y));
	...
	public getPointAt(t: number, result?: Point): Point {
		...
		if (t == 1) {
			...
			if (seg.type == CurveType.Straight) {
				result.x = ToolSet.lerp(
					this._points[seg.ptStart].x,
					this._points[seg.ptStart + 1].x,
					t
				);
				result.y = ToolSet.lerp(
					this._points[seg.ptStart].y,
					this._points[seg.ptStart + 1].y,
					t
				);

				return result;
			} else if (
				seg.type == CurveType.Bezier ||
	...
	public getAnchorsInSegment(
		...
		for (var i: number = 0; i < seg.ptCount; i++)
			points.push(
				new Point(
					this._points[seg.ptStart + i].x,
					this._points[seg.ptStart + i].y
				)
			);
	...
	public getPointsInSegment(
		...
		if (seg.type == CurveType.Straight) {
			...
			for (var j: number = 0; j <= SmoothAmount; j++) {
				...
				if (t > t0 && t < t1) {
					points.push(
						func.call(this, seg.ptStart, seg.ptCount, t, new Point())
					);
					if (ts) ts.push(t);
				}
			}
