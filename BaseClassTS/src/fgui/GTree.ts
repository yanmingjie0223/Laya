...
export class GTree extends GList {
	...
	public getSelectedNodes(result?: Array<GTreeNode>): Array<GTreeNode> {
		if (!result) result = new Array<GTreeNode>();
	...
	private createCell(node: GTreeNode): void {
		var child: GComponent = <GComponent>(
			this.getFromPool(node._resURL ? node._resURL : this.defaultItem)
		);
		...
		if (node.isFolder)
			child.on(Event.MOUSE_DOWN, this, this.__cellMouseDown);
...
const s_list: number[] = new Array<number>();
