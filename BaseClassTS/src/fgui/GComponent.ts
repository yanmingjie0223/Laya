...
export class GComponent extends GObject {
	...
	constructor() {
		...
		this._cKey = GObjectType.GComponent;
		this._children = [];
		this._controllers = [];
	...
	public get displayListContainer(): Sprite {
		...
		if (index >= 0 && index < this._children.length) {
			...
			if (child.inContainer) {
				...
				if (this._childrenRenderOrder == ChildrenRenderOrder.Arch)
					timer.callLater(this, this.buildNativeDisplayList);
			}
	...
	public getChildAt(index: number): GObject | null {
		if (index >= 0 && index < this._children.length)
	...
	public childStateChanged(child: GObject): void {
		...
		if (
			...
			child.displayObject != this._displayObject.mask
		) {
	...
	public adjustRadioGroupDepth(obj: GObject, c: Controller): void {
		...
		for (i = 0; i < cnt; i++) {
			...
			if (child == obj) {
				...
				child['_cKey'] === GObjectType.GButton &&
		...
		if (myIndex < maxIndex) {
			...
			if (this._applyingController)
				this._children[maxIndex].handleControllerChanged(
	...
	public get mask(): Sprite {
		return this._mask;
	}

	public set mask(value: Sprite) {
		this.setMask(value, false);
	}

	public setMask(value: Sprite, reversed: boolean): void {
	...
	protected handleSizeChanged(): void {
		...
		else if (this._displayObject.scrollRect) this.updateMask();
	...
	public setBoundsChangedFlag(): void {
		...
		if (!this._boundsChanged) {
			...
			timer.callLater(this, this.__render);
		}
	}
	...
	protected updateBounds(): void {
		...
		if (len > 0) {
			...
			var ar: number = Number.NEGATIVE_INFINITY,
	...
	public childSortingOrderChanged(
		child: GObject,
		oldValue: number,
		newValue: number
	): void {
	...
	public setup_afterAdd(buffer: ByteBuffer, beginPos: number): void {
		...
		if (pageController != -1 && this._scrollPane)
			this._scrollPane.pageController = this._parent.getControllerAt(
				pageController
			);
