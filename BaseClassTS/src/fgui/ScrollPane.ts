...
export class ScrollPane {
	...
	private _alignContainer?: Sprite;
	...
	public setup(buffer: ByteBuffer): void {
		...
		if (scrollBarDisplay != ScrollBarDisplayType.Hidden) {
			if (
				this._scrollType == ScrollType.Both ||
				this._scrollType == ScrollType.Vertical
			) {
				var res: string = vtScrollBarRes
					? vtScrollBarRes
			...
			if (
				this._scrollType == ScrollType.Both ||
				this._scrollType == ScrollType.Horizontal
			) {
		...
		if (headerRes) {
			...
			if (!this._header)
				throw new Error(
					"FairyGUI: cannot create scrollPane this.header from " + headerRes
				);
		}
		...
		if (footerRes) {
			...
			if (!this._footer)
				throw new Error(
					"FairyGUI: cannot create scrollPane this.footer from " + footerRes
				);
		}
	...
	public dispose(): void {
		...
		if (this._tweening != 0) timer.clear(this, this.tweenUpdate);
	...
	public scrollToView(
		...
		if (this._overlapSize.y > 0) {
			...
			if (setFirst || rect.y <= this._yPos || rect.height >= this._viewSize.y) {
				if (this._pageMode)
					this.setPosY(
						Math.floor(rect.y / this._pageSize.y) * this._pageSize.y,
						ani
					);
		...
		if (this._overlapSize.x > 0) {
			...
			if (setFirst || rect.x <= this._xPos || rect.width >= this._viewSize.x) {
				if (this._pageMode)
					this.setPosX(
						Math.floor(rect.x / this._pageSize.x) * this._pageSize.x,
						ani
					);
	...
	public lockFooter(size: number): void {
		...
		if (
			...
			this._tweenChange[this._refreshBarAxis] =
				-max - this._tweenStart[this._refreshBarAxis];
			this._tweenDuration.setTo(TWEEN_TIME_DEFAULT, TWEEN_TIME_DEFAULT);
			this.startTween(2);
		}
	}
	...
	public adjustMaskContainer(): void {
		...
		if (this._dontClipMargin) {
			...
			else mx = this._owner.margin.left;
	...
	public setSize(aWidth: number, aHeight: number): void {
		...
		if (this._hzScrollBar) {
			...
			if (this._vtScrollBar) {
				...
				this._hzScrollBar.width =
					aWidth - this._scrollBarMargin.left - this._scrollBarMargin.right;
		...
		if (this._vtScrollBar) {
			...
			if (this._hzScrollBar)
				...
				this._vtScrollBar.height =
					aHeight - this._scrollBarMargin.top - this._scrollBarMargin.bottom;
	...
	public changeContentSizeOnScrolling(
		deltaWidth: number,
		deltaHeight: number,
		deltaPosX: number,
		deltaPosY: number
	...
	private handleSizeChanged(): void {
		...
		if (this._refreshBarAxis != null) {
			...
			if (this._refreshBarAxis == "x") {
				this._container.pos(
					ToolSet.clamp(this._container.x, -max, this._headerLockedSize),
					ToolSet.clamp(this._container.y, -this._overlapSize.y, 0)
				);
			} else {
				this._container.pos(
					ToolSet.clamp(this._container.x, -this._overlapSize.x, 0),
	...
	private posChanged(ani: boolean): void {
		...
		timer.callLater(this, this.refresh);
	}
	...
	private refresh(): void {
		...
		timer.clearCallLater(this, this.refresh);
		...
		if (this._aniFlag == 1 && !this._dragged) {
			...
			if (posX != this._container.x || posY != this._container.y) {
				...
				this._tweenChange.setTo(
					posX - this._tweenStart.x,
					posY - this._tweenStart.y
				);
				this.startTween(1);
	...
	private __mouseDown(): void {
		...
		var pt: Point = this._owner.globalToLocal(
			stage.mouseX,
			stage.mouseY,
			s_vec2
		);
		...
		this._lastTouchGlobalPos.setTo(stage.mouseX, stage.mouseY);
		this._isHoldAreaDone = false;
		this._velocity.setTo(0, 0);
		this._velocityScale = 1;
		this._lastMoveTime = timer.currTimer / 1000;
	...
	private __mouseMove(): void {
		...
		if (
			(ScrollPane.draggingPane && ScrollPane.draggingPane != this) ||
		...
		var newPosX: number = Math.floor(
			this._containerPos.x + pt.x - this._beginTouchPos.x
		);
		var newPosY: number = Math.floor(
			this._containerPos.y + pt.y - this._beginTouchPos.y
		);
			...
			if (newPosY > 0) {
				if (!this._bouncebackEffect) this._container.y = 0;
					...
					this._container.y = Math.floor(
						Math.min(newPosY * 0.5, this._header.maxHeight)
					);
				else
					this._container.y = Math.floor(
						Math.min(newPosY * 0.5, this._viewSize.y * PULL_RATIO)
					);
				...
				if (!this._bouncebackEffect) this._container.y = -this._overlapSize.y;
					...
					this._container.y = Math.floor(
						Math.max(
							(newPosY + this._overlapSize.y) * 0.5,
			...
			if (newPosX > 0) {
				if (!this._bouncebackEffect) this._container.x = 0;
					...
					this._container.x = Math.floor(
						Math.min(newPosX * 0.5, this._header.maxWidth)
					);
				else
					this._container.x = Math.floor(
						Math.min(newPosX * 0.5, this._viewSize.x * PULL_RATIO)
					);
				...
				if (!this._bouncebackEffect) this._container.x = -this._overlapSize.x;
					...
					this._container.x = Math.floor(
						Math.max(
							(newPosX + this._overlapSize.x) * 0.5,
		...
		var frameRate: number =
			stage.frameRate == Stage.FRAME_SLOW ? 30 : 60;
		var now: number = timer.currTimer / 1000;
		...
		if (deltaTime != 0) {
			...
			this._velocity.x = ToolSet.lerp(
				this._velocity.x,
				(deltaPositionX * 60) / frameRate / deltaTime,
				deltaTime * 10
			);
			this._velocity.y = ToolSet.lerp(
				this._velocity.y,
		 ...
		 */
		var deltaGlobalPositionX: number =
			this._lastTouchGlobalPos.x - stage.mouseX;
		var deltaGlobalPositionY: number =
			this._lastTouchGlobalPos.y - stage.mouseY;
		if (deltaPositionX != 0)
	...
	private __mouseUp(): void {
		...
		if (flag) {
			this._tweenChange.setTo(
				sEndPos.x - this._tweenStart.x,
				sEndPos.y - this._tweenStart.y
			);
			if (
				this._tweenChange.x < -UIConfig.touchDragSensitivity ||
				this._tweenChange.y < -UIConfig.touchDragSensitivity
			) {
			...
			if (!this._inertiaDisabled) {
				var frameRate: number =
					stage.frameRate == Stage.FRAME_SLOW ? 30 : 60;
				var elapsed: number =
					(timer.currTimer / 1000 - this._lastMoveTime) * frameRate - 1;
				if (elapsed > 1) {
					var factor: number = Math.pow(0.833, elapsed);
					this._velocity.x = this._velocity.x * factor;
					this._velocity.y = this._velocity.y * factor;
	...
	private __mouseWheel(evt: Event): void {
		...
		if (this._overlapSize.x > 0 && this._overlapSize.y == 0) {
			...
			else this.setPosX(this._xPos + this._mouseWheelStep * delta, false);
		} else {
			if (this._pageMode)
				this.setPosY(this._yPos + this._pageSize.y * delta, false);
			else this.setPosY(this._yPos + this._mouseWheelStep * delta, false);
		}
	}
		...
		if (this._vtScrollBar)
			this._vtScrollBar.setScrollPerc(
				this._overlapSize.y == 0
					? 0
					: ToolSet.clamp(-this._container.y, 0, this._overlapSize.y) /
							this._overlapSize.y
			);
		...
		if (this._hzScrollBar)
			this._hzScrollBar.setScrollPerc(
				this._overlapSize.x == 0
					? 0
					: ToolSet.clamp(-this._container.x, 0, this._overlapSize.x) /
							this._overlapSize.x
			);
	...
	private getLoopPartSize(division: number, axis: string): number {
		return (
			(this._contentSize[axis] +
				(axis == "x"
					? (<GList>this._owner).columnGap
					: (<GList>this._owner).lineGap)) /
			division
		);
	}
	...
	private alignPosition(pos: Point, inertialScrolling: boolean): void {
		if (this._pageMode) {
			...
			var pt: Point = this._owner.getSnappingPositionWithDir(
				-pos.x,
				-pos.y,
				xDir,
				yDir,
	...
	private alignByPage(
		...
		if (pos > 0) page = 0;
			...
			var change: number = inertialScrolling
				...
				: pos - this._container[axis];
			var testPageSize: number = Math.min(
				this._pageSize[axis],
				this._contentSize[axis] - (page + 1) * this._pageSize[axis]
			);
		...
		if (inertialScrolling) {
			...
			var startPage: number = Math.floor(
				-this._containerPos[axis] / this._pageSize[axis]
			);
			if (
				Math.abs(page - startPage) > 1 &&
				Math.abs(oldPage - startPage) <= 1
			) {
	...
	private updateTargetAndDuration2(pos: number, axis: string): number {
		...
		if (pos > 0) pos = 0;
			...
			if (Browser.onMobile)
				v2 *= 1136 / Math.max(stage.width, stage.height);
	...
	private fixDuration(axis: string, oldChange: number): void {
		if (
			...
			Math.abs(this._tweenChange[axis]) >= Math.abs(oldChange)
		)
			return;

		var newDuration: number =
			Math.abs(this._tweenChange[axis] / oldChange) * this._tweenDuration[axis];
	...
	private startTween(type: number): void {
		...
		this._tweening = type;
		timer.frameLoop(1, this, this.tweenUpdate);
	...
	private killTween(): void {
		...
		this._tweening = 0;
		timer.clear(this, this.tweenUpdate);
	...
	private checkRefreshBar(): void {
		...
		if (this._footer) {
			...
			if (pos < -max || (max == 0 && this._footerLockedSize > 0)) {
				...
				if (max > 0)
					pt[this._refreshBarAxis] =
						pos + this._contentSize[this._refreshBarAxis];
				else
					pt[this._refreshBarAxis] = Math.max(
						Math.min(
							pos + this._viewSize[this._refreshBarAxis],
							this._viewSize[this._refreshBarAxis] - this._footerLockedSize
						),
						this._viewSize[this._refreshBarAxis] -
							this._contentSize[this._refreshBarAxis]
					);
	...
	private runTween(axis: string): number {
		...
		if (this._tweenChange[axis] != 0) {
			this._tweenTime[axis] += timer.delta / 1000;
			if (this._tweenTime[axis] >= this._tweenDuration[axis]) {
				newValue = this._tweenStart[axis] + this._tweenChange[axis];
				this._tweenChange[axis] = 0;
			} else {
				var ratio: number = easeFunc(
					this._tweenTime[axis],
					this._tweenDuration[axis]
				);
				newValue =
					this._tweenStart[axis] + Math.floor(this._tweenChange[axis] * ratio);
			}
			...
			if (this._tweening == 2 && this._bouncebackEffect) {
				if (
					(newValue > 20 + threshold1 && this._tweenChange[axis] > 0) ||
					...
					(newValue < threshold2 - 20 && this._tweenChange[axis] < 0) ||
...
const s_rect: Rectangle = new Rectangle();
const sEndPos: Point = new Point();
const sOldChange: Point = new Point();
