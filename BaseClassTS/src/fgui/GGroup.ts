...
export class GGroup extends GObject {
	...
	private updateBounds(): void {
		timer.clearCallLater(this, this.ensureBoundsCorrect);

		var cnt: number = this._parent.numChildren;
		var i: number;
		var child: GObject;
		var ax: number = Number.POSITIVE_INFINITY,
		...
		for (i = 0; i < cnt; i++) {
			...
			if (
				child.group != this ||
				(this._excludeInvisibles && !child.internalVisible3)
			)
				continue;
	...
	public resizeChildren(dw: number, dh: number): void {
		if (
			this._layout == GroupLayoutType.None ||
			(this._updating & 2) != 0 ||
			!this._parent
		)
			return;
			...
			for (i = 0; i < cnt; i++) {
				...
				if (this._totalSize > 0)
					child._sizePercentInGroup =
						(this._layout == 1 ? child.width : child.height) / this._totalSize;
		...
		if (this._layout == 1) {
			...
			if (this._mainChildIndex != -1 && remainSize >= this._totalSize) {
				child = this._parent.getChildAt(this._mainChildIndex);
				child.setSize(
					remainSize - (this._totalSize - this._mainGridMinSize),
					child._rawHeight + dh,
					true
				);
			...
			for (i = 0; i < cnt; i++) {
				...
				if (!priorHandled || i != this._mainChildIndex) {
					child.setSize(
						Math.round(
							(child._sizePercentInGroup / remainPercent) * remainSize
						),
						child._rawHeight + dh,
						true
					);
