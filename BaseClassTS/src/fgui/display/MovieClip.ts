...
export class MovieClip extends Image {
	...
	public advance(timeInMiniseconds: number): void {
		...
		while (true) {
			...
			if (this._frame == beginFrame && this._reversed == beginReversed) {
				...
				timeInMiniseconds -=
					Math.floor(timeInMiniseconds / roundTime) * roundTime; //跳过
			}
		}
	...
	private update(): void {
		...
		if (this._status == 1) {
			...
			//play end
			if (this._endHandler) {
				var handler: Handler = this._endHandler;
			...
			if (this._frame == this._end) {
				if (this._times > 0) {
					this._times--;
					if (this._times == 0) this._status = 2;
					//ending
	...
	private checkTimer(): void {
		...
		else timer.clear(this, this.update);
	}
