...
export function fillImage(
	...
	else if (amount >= 0.9999) return [0, 0, w, 0, w, h, 0, h];
...
function fillHorizontal(
	...
	else return [w, 0, w, h, w - w2, h, w - w2, 0];
}
...
function fillVertical(
	...
	else return [0, h, w, h, w, h - h2, 0, h - h2];
}
...
function fillRadial90(
	...
	if (
		(clockwise &&
			(origin == FillOrigin.TopRight || origin == FillOrigin.BottomLeft)) ||
		(!clockwise &&
			(origin == FillOrigin.TopLeft || origin == FillOrigin.BottomRight))
	) {
		amount = 1 - amount;
	}
	...
	switch (origin) {
		case FillOrigin.TopLeft:
			if (clockwise) {
				if (h2 <= h) points = [0, 0, w, h2, w, 0];
				else points = [0, 0, w * (1 - v2), h, w, h, w, 0];
			} else {
				if (h2 <= h) points = [0, 0, w, h2, w, h, 0, h];
				else points = [0, 0, w * (1 - v2), h, 0, h];
			}
			break;

		case FillOrigin.TopRight:
			if (clockwise) {
				if (h2 <= h) points = [w, 0, 0, h2, 0, h, w, h];
				else points = [w, 0, w * v2, h, w, h];
			} else {
				if (h2 <= h) points = [w, 0, 0, h2, 0, 0];
				else points = [w, 0, w * v2, h, 0, h, 0, 0];
			}
			break;

		case FillOrigin.BottomLeft:
			if (clockwise) {
				if (h2 <= h) points = [0, h, w, h - h2, w, 0, 0, 0];
				else points = [0, h, w * (1 - v2), 0, 0, 0];
			} else {
				if (h2 <= h) points = [0, h, w, h - h2, w, h];
				else points = [0, h, w * (1 - v2), 0, w, 0, w, h];
			}
		...
		case FillOrigin.BottomRight:
			if (clockwise) {
				if (h2 <= h) points = [w, h, 0, h - h2, 0, h];
				else points = [w, h, w * v2, 0, 0, 0, 0, h];
			} else {
				if (h2 <= h) points = [w, h, 0, h - h2, 0, 0, w, 0];
				else points = [w, h, w * v2, 0, w, 0];
			}
			break;
	}
	return points;
	...
	switch (origin) {
		case FillOrigin.Top:
			if (amount <= 0.5) {
				...
				if (clockwise) points.push(w, h, w, 0);
				else {
		...
		case FillOrigin.Right:
			if (amount <= 0.5) {
				...
				if (clockwise) points.push(0, h, w, h);
				else {
					movePoints(points, 0, h / 2);
					points.push(0, 0, w, 0);
				}
	...
	switch (origin) {
		case FillOrigin.Top:
			if (amount <= 0.5) {
				...
				if (clockwise) points.push(w, h, w, 0, w / 2, 0);
				else {
					movePoints(points, w / 2, 0);
		...
		case FillOrigin.Right:
			if (amount <= 0.5) {
				...
				if (clockwise) points.push(0, h, w, h, w, h / 2);
				else {
					movePoints(points, 0, h / 2);
