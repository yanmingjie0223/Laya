...
export class GLabel extends GComponent {
	...
	public set editable(val: boolean) {
		if (this._titleObject) (<GTextInput>this._titleObject).editable = val;
	}

	public get editable(): boolean {
		if (this._titleObject['_cKey'] === GObjectType.GTextInput) {
	...
	public setup_afterAdd(buffer: ByteBuffer, beginPos: number): void {
		...
		if (buffer.readBool()) {
			...
			if (input instanceof GTextInput) {
				...
				iv = buffer.getInt32();
				if (iv != 0) {
					if (iv == 4) input.keyboardType = Input.TYPE_NUMBER;
					else if (iv == 3) input.keyboardType = Input.TYPE_URL;
				}
