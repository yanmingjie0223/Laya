...
export class GLoader extends GObject {
	...
	constructor() {
		...
		this._cKey = GObjectType.GLoader;
		this._url = "";
	...
	protected loadExternal(): void {
		AssetProxy.inst.load(
			this._url,
			Handler.create(this, this.__getResCompleted),
			null,
			Loader.IMAGE
		);
	}
	...
	private __getResCompleted(tex: Texture): void {
		if (tex != null) this.onExternalLoadSuccess(tex);
		else this.onExternalLoadFailed();
	}
	...
	private setErrorState(): void {
		...
		if (!this._errorSign) {
			if (UIConfig.loaderErrorSign != null) {
				this._errorSign = GLoader._errorSignPool.getObject(
					UIConfig.loaderErrorSign
				);
			}
		}
	...
	private updateLayout(): void {
		...
		if (this._content2) this._content2.setXY(nx, ny);
		else this._content.pos(nx, ny);
	}
