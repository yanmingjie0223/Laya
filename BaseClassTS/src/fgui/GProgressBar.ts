...
export class GProgressBar extends GComponent {
	...
	constructor() {
		...
		this._cKey = GObjectType.GProgressBar;
	...
	public tweenValue(value: number, duration: number): GTweener {
		...
		this._value = value;
		return GTween.to(oldValule, this._value, duration)
	...
	private setFillAmount(bar: GObject, percent: number): boolean {
		if (
			(bar instanceof GImage || bar instanceof GLoader) &&
			bar.fillMethod != FillMethod.None
		) {
