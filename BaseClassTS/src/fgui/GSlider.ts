...
export class GSlider extends GComponent {
	...
	constructor() {
		...
		this._cKey = GObjectType.GSlider;
	...
	private updateWithPercent(percent: number, evt?: Event): void {
		percent = ToolSet.clamp01(percent);
		if (evt) {
			var newValue: number = ToolSet.clamp(
			...
			if (this._wholeNumbers) {
				...
				percent = ToolSet.clamp01(
					(newValue - this._min) / (this._max - this._min)
				);
			}
		...
		if (!this._reverse) {
			...
			if (this._barObjectH) {
				this._barObjectH.width = Math.round(fullWidth * percent);
				this._barObjectH.x =
					this._barStartX + (fullWidth - this._barObjectH.width);
			}
			if (this._barObjectV) {
				this._barObjectV.height = Math.round(fullHeight * percent);
				this._barObjectV.y =
	...
	private __gripMouseDown(evt: Event): void {
		...
		this._clickPos = this.globalToLocal(stage.mouseX, stage.mouseY);
		this._clickPercent = ToolSet.clamp01(
		...
		stage.on(Event.MOUSE_OUT, this, this.__gripMouseOut);
	}

	private __gripMouseMove(evt: Event): void {
		if (!this.canDrag) {
			return;
		}
	...
	private __gripMouseUp(evt: Event): void {
		...
		stage.off(Event.MOUSE_OUT, this, this.__gripMouseOut);
	}

	private __gripMouseOut(evt: Event): void {
		this.__gripMouseUp(null);
	}

	private __barMouseDown(evt: Event): void {
		if (!this.changeOnClick) return;

		var pt: Point = this._gripObject.globalToLocal(
			evt.stageX,
			evt.stageY,
			s_vec2
		);
		var percent: number = ToolSet.clamp01(
			(this._value - this._min) / (this._max - this._min)
		);
		var delta: number;
