...
export class GTextInput extends GTextField {
	...
	constructor() {
		...
		this._cKey = GObjectType.GTextInput;
	}

	protected createDisplayObject(): void {
		this._displayObject = this._input = new Input();
	...
	public set font(value: string) {
		if (value) this._input.font = value;
		else this._input.font = UIConfig.defaultFont;
	}
	...
	public set password(value: boolean) {
		if (value) this._input.type = "password";
		else this._input.type = "text";
	}
	...
	public setup_beforeAdd(buffer: ByteBuffer, beginPos: number): void {
		...
		iv = buffer.getInt32();
		if (iv != 0) {
			if (iv == 4) this.keyboardType = Input.TYPE_NUMBER;
			else if (iv == 3) this.keyboardType = Input.TYPE_URL;
		}
